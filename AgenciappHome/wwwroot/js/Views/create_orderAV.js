...
$(document).ready(function () {
    ...
    function aux(idmayorista) {
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#Transitaria').empty();
                $('#Transitaria').append(new Option("TRANSCARGO", "TRANSCARGO"));
                $('#Transitaria').append(new Option("AEROVARADERO", "AEROVARADERO"));
                $('#Transitaria').append(new Option("PALCO", "PALCO"));
                $('#Transitaria').append(new Option("CUBANACAN", "CUBANACAN"));

                $('#Type').empty();
                $('#Type').append(new Option("AEREO", "AEREO"));
                $('#Type').append(new Option("MARITIMO", "MARITIMO"));
    ...
    function getMayorista(category) {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.length != 0) {
                    var byTransferencia = null;
                    for (var i = 0; i < data.length; i++) {
                        $("[name='selectMayorista']").append(
                            new Option(data[i].name, data[i].idWholesaler)
                        );
                        if (data[i].byTransferencia) {
                            byTransferencia = data[i];
                        }
                    }

                    if (byTransferencia) {
                        $("[name='selectMayorista']").val(byTransferencia.idWholesaler).trigger("change");
                    }
                }
            },
            failure: function (response) {
    ...
    $("#zc").steps({
        ...
        onFinishing: function (event, currentIndex) {
            ...
            if (isCubiq == "False" && $("#selectMayorista").val() == "") {
                showWarningMessage("Atención", "El campo mayorista es obligatorio.");
            } else return sendOrder();
        },
    ...
    $("#AWB").select2({});
    $('#Type').select2({ placeholder: "Tipo de trámite" });
    $('#Transitaria').select2({ placeholder: "Seleccione una transitaria" });
    $("#select-aduanal").select2({ placeholder: "Seleccione valores aduanales" });
    $("#select-seguros").select2({ placeholder: "Seleccione una opcion" });
    ...
    var validateEditarContacto = function () {
        if ($("#inputContactName").val() == "") {
            ...
            return false;
        } else if ($("#provincia").val() == "" || $("#provincia").val() == null) {
            showWarningMessage(
                "Atención",
                "El campo Provincia no puede estar vacío."
            );
            return false;
        } else if ($("#municipio").val() == "" || $("#municipio").val() == null) {
            showWarningMessage(
                "Atención",
    ...
    const calcMoneyPayment = function () {
        displayValorAduanal();
    ...
    $('#AWB').on('change', async function () {
        ...
        var guide = await GetGuideByNumber(value);
        var province = "";
        switch (guide.type) {
            case "HAV":
                province = "Habana";
        ...
        swal({
            title: "Confirmación de Guia Aerea",
            text: `¿Está seguro que desea seleccionar la guia # ${guide.noGuia} - ${province} ?`,
            type: "warning",
    ...
    var sendOrder = function () {
        ...
        //Valido la orden
        const validate = validateCreateOrder();
        if (!isSend && validate) {
            isSend = true;
            ...
            for (var i = 1; i < tBody.rows.length; i++) {
                ...
                listPaquetes[index][10] = $(fila).attr('data-prodautom'); // saber si es producto automatico
                index += 1;
            }
            ...
            var datosOrden = [
                ...
                tipopagos, //                                                               4
                listPaquetes, //                                                            5
                idValoresAduanales, // listado de valores aduanales                                         6
                $("#Price").val().replace(",", "."), //                                     7
                ...
                $('#Type').val(), //                                                       33
                $('#Transitaria').val(), //                                                34
                $('#CargoAdicional').val(), //                                             35
                $('#CargoAdicionalDescription').val(), //                                  36
                idInsuranceValues, //                                                      37
                $('#InsuranceValue').val(), //                                             38
                listProducts, //                                                           39
                $('#ena_passport').val(), //                                               40
                $('#NoAduana').is(':checked'), //                                                   41
                $('#RecogidaAlmacen').is(':checked'), //                                            42
            ];
        ...
        else console.log("is send");
        return true;
    };

    var guardarEnvio = function (datosOrden) {
        //Guardo el envio
        $.ajax({
            type: "POST",
            url: "/OrderCubiq/CreateOrderAV",
    ...
    var validateCreateOrder = function () {
        ...
        if ($("#Price").val() == "") {
            ...
            showWarningMessage(
                "Atención",
                "El campo No. de Guía no puede estar vacio."
            );
            return false;
        }
        else if (!$('#Type').val()) {
        ...
        else if (!$('#Transitaria').val()) {
            showWarningMessage(
                "Atención",
                "El campo Transitaria no puede estar vacío."
            );
            return false;
        }
        const splitLasName = $("#inputContactLastName").val().trim().split(' ');
    ...
    var addPaqueteToTable = function (aduanalId, descripcion, type, pesoLb, precio, rawMaterial, aduanal, isProdAutom, reference) {
    ...
    $(
        "#OtrosCostos,#txtNameVA,#check_credito,#pagoCash,#pagoZelle,#pagoCheque,#pagoCredito,#pagoTransf,#pagoWeb,#Descuento,#Price,#HandlingAndTransportation_Sale,#CargoAdicional"
    )
        .on("keyup", calcMoney)
        .on("change", calcMoney);
    ...
    $("#Type").on("change", function () {
        ...
        // obtener nombre de mayorista
        var nombreMayorista = $('#selectMayorista option:selected').text();
        if (agencyId == agencyFly || nombreMayorista == 'Fly Away Travel') {
            if ($(this).val() == "MARITIMO") {
                $('#Transitaria').empty();
                $('#Transitaria').append(new Option("TRANSCARGO", "TRANSCARGO"));
                $('#Transitaria').append(new Option("PALCO", "PALCO"));
                $('#Transitaria').val('TRANSCARGO').trigger('change');
            }
            else if ($(this).val() == "AEREO") {
                $('#Transitaria').empty();
                $('#Transitaria').append(new Option("PALCO", "PALCO"));
                $('#Transitaria').val('PALCO').trigger('change');
            }
        }
        else if (agencyId == '13241095-D8F3-400D-8D62-75B40FC33A0F'.toLowerCase() || nombreMayorista == 'Rapi Cargo') {
            if ($(this).val() == "MARITIMO") {
                $('#Transitaria').empty();
                $('#Transitaria').append(new Option("TRANSCARGO", "TRANSCARGO"));
                $('#Transitaria').val('TRANSCARGO').trigger('change');
    ...
    $('#ena_checkbox').on('change', function () {
        if ($(this).is(':checked')) {
            $('#ena_passport').prop('disabled', false);
        }
        else {
            $('#ena_passport').prop('disabled', true);
            $('#ena_passport').val("");
        }
    });
    ...
    function getTasaCambio() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#show-tasacambio').html(tasaCambio.toFixed(2));
