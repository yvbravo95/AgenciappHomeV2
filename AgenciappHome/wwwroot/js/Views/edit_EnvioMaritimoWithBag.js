$(document).ready(function () {
    ...
    var calcMoney = function () {
        ...
        if (blockRight == "True") {
            ...
            productsSelected.forEach((element) => {
                var row = $('tr[data-productId="' + element.idProducto + '"]');
                var cant = parseInt($(row).first()[0].children[1].innerText);
            ...
            if ($("#checkpago").is(":checked")) {
                ...
                var balanceValue =
                    precioTotalValue - parseFloat($("#ValorPagado").val());
                if ($("#check_credito").is(":checked")) {
                    if (
    ...
    var addProductToTable = function (
        cant,
        descripcion,
        id = null,
        productoBodegaId = null,
        peso = null,
        precio = "0",
        package = false
    ) {
        ...
        $(row).attr("data-productId", productoBodegaId);
        $(row).attr("data-id", id);

        if (productoBodegaId != "" && productoBodegaId != "00000000-0000-0000-0000-000000000000" && productoBodegaId != null) {
        ...
        cell1.append(package ? "PAQUETE" : "PRODUCTO");
        ...
        btnEdit.on("click", function () {
            if (cell1.html() == "PAQUETE") {
                cell1.html(
                    `<input type="checkbox" name="cell1" class="form-control checkbox-big" checked />`
                );
            }
            else {
                cell1.html(
                    `<input type="checkbox" name="cell1" class="form-control checkbox-big" />`
                );
            }

            cell2.html(
                "<input type='number' name='cell2' class='form-control'  data-preVal=" +
                cell2.html() +
                " value='" +
                cell2.html() +
                "'/>"
            ...
            if (!isProductBodega) {
                ...
                cell7.html(
                    "<input type='number' name='cell7' step='0.1' min='0' data-preVal=" +
                    cell7.html() +
        ...
        btnRemove.on("click", function () {
            var id = $(row).attr("data-productId");
            if (id != null && id != "") {
        ...
        btnConfirm.on("click", function () {
            if (validateEditProduct()) {
                var check = $("[name='cell1']").is(":checked");
                cell1.html(check ? "PAQUETE" : "PRODUCTO");
                var description = $("[name='cell5']").val();
                var cant = $("[name='cell2']").val();
                var prevCant = parseFloat($("[name='cell2']").data("preval"));
                ...
                if (!isProductBodega) {
                    ...
                    $(cell3).html(parseFloat(precio) * parseInt($(cell2).html()));
                }
        ...
        if (id != null) {
            cell6.data("id", id);
            cell6.append(btnEdit.add(btnConfirm));
        } else {
    ...
    var sendOrder = function () {
        if (validateCreateOrder()) {
            ...
            for (var i = 0; i < tBody.rows.length - 1; i++) {
                ...
                listProduct[i][0] = $(fila.children[1]).html();
                listProduct[i][1] = $(fila.children[2]).html();
                listProduct[i][2] = $(fila.children[5]).data("id");
                listProduct[i][3] = $(fila).attr("data-productId");
                listProduct[i][4] = $(fila.children[3]).html();
                listProduct[i][5] = $(fila.children[0]).html();
            }
            ...
            if ($("#checkpago").is(":checked")) {
                ...
                if ($("#ValorPagado").val()) {
                    ...
                    valorespagado = [valorPagado];
                }
                tipopagos = [$(".hide-search-pago option:selected").text()];
                notas = [$("#NotaPago").val()];
            }
    ...
    var p = JSON.parse(productos_list);
    for (var i = 0; i < p.length; i++) {
        addProductToTable(
            p[i].cantidad,
            p[i].Tipo,
            p[i].Id,
            p[i].productoBodegaId,
            p[i].peso,
            p[i].precioVentaReferencial,
            false
        );
    }
    var bagProducts = JSON.parse(bags_list);
    for (var i = 0; i < bagProducts.length; i++) {
        addProductToTable(
            bagProducts[i].cantidad,
            bagProducts[i].tipo,
            bagProducts[i].Id,
            bagProducts[i].productoBodegaId,
            bagProducts[i].peso,
            bagProducts[i].precioVentaReferencial,
            true
        );
    }


    $("#mayorista").val(may).trigger("change");
    ...
    async function InitSelectProduct() {
        ...
        //Verifico si existe algun producto de la bodega en la tabla
        var tBody = $("#tblProductos > TBODY")[0];
        ...
        if (code != "" && code != null) {
            ...
            if (productAux != null) {
                ...
                if (
                    ...
                    var row = $('[data-productId="' + productAux.productId + '"]');
    ...
    async function barCodeSelect(code) {
        if (code != "" && code != null) {
            var productAux = null;
            productos.forEach(function (item) {
                if (item.barCode == code) {
            ...
            if (productAux != null) {
                ...
                if (
                    ...
                    var row = $('[data-productId="' + productAux.productId + '"]');
                    var childrens = $(row).children("td");
                    var input = $(childrens.first());
                    $(input).html(parseInt($(input).html()) + 1);
                    var price =
                        product.precioVentaReferencial * parseFloat($(input).html());
                    $(childrens[3]).html(price);
                ...
                toastr.info("El producto no ha sido encontrado");
            }
        }
    }
    ...
    function UpdatePriceProductsTable() {
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if ($(row).attr('class') != 'newRow') {
                if ($(row).attr("data-productId") == "" || $(row).attr("data-productId") == null) {
                    var elementCant = $($(row).children("td")[1]).html();
                    var elementDescription = $($(row).children("td")[2]).html();
                    var elementPrice = $(row).children("td")[4];
                    var elementPeso = $(row).children("td")[3];
                    var price = 0;
    ...
    $("#EnablePriceLbDuradero").trigger("change");
});
