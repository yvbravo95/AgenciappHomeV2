    ...
    $("#zc").steps({
        ...
        onStepChanging: function (event, currentIndex, newIndex) {
            ...
            if (newIndex == 1) {
                ...
                var validContacto = validateEditarContacto();
                if(!validContacto){
                    return false;
                }
                getMayorista("Cubiq"); 
...
/*********************************************/
    ...
    var sendOrder = function () {
        ...
        if (validateCreateOrder()) {
            ...
            for (var i = 1; i < tBody.rows.length; i++) {//filas de cada producto (excepto la 1era)
                ...
                listPaquetes[index][6] = $(fila.children[0]).data("id");
                index += 1;
            }
    ...
    var addProductToTable = function (pesoLb, largo, alto, ancho, precio, descripcion, id = 0) {
    ...
    if(productos != [] )
    {
        for (let index = 0; index < productos.length; index++) {
            ...
            addProductToTable(element[0],element[1],element[2],element[3],element[4],element[5],element[6]);          
        }
    }
    if(pagos.length == 1)
    {
    ...
    {
        $("#selectMayorista").val(idMayoristabyTransferencia);
    }
