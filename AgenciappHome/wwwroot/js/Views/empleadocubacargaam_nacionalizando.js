...
$(window).on("load", () => {
    ...
    tableIniciada = t.DataTable({
        ...
        ajax: {
            "url": `/empleadocubacargaam/GetData?status=Nacionalizando`,
            "type": 'POST',
            "dataType": "json",
...
function LoadEvents() {
    ...
    $(document).on("change", ".order-select", function () {
        ...
        if (cantSelect > 0) {
            $('#btn_transitoCuba').removeClass('hidden');
        }
        else {
            $('#btn_transitoCuba').addClass('hidden');
        }
    });

    $('#btn_transitoCuba').on('click', function () {
    ...
    $('#btnModalTransferir').on('click', function () {
        ...
        if (!almacen) {
            swal('Seleccione un almacen', '', 'warning');
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    swal("¡Correcto!", "Las órdenes seleccionadas se han transferido", "success");
                    ...
                    console.log(data);
                    swal("¡Error!", "Ha ocurrido un error al intentar transferir los paquetes", "error");
                }
            },
            error: function (data) {
                console.log(data);
