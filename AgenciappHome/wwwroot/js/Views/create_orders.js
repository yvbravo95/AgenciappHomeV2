$(document).ready(function () {
    ...
    var calcMoney = function () {
        ...
        var precioLb = parseFloat($("#PriceLb").val().replace(",", "."));
        var oCargos = parseFloat($("#OtrosCostos").val().replace(",", "."));
        var pagado = parseFloat($("#ValorPagado").val().replace(",", "."));
    ...
    var addProductToTable = function (cant, tipo, color, tallamarca, descripcion) {
        //Get the reference of the Table's TBODY element.
        var tBody = $("#tblProductos > TBODY")[0];
    ...
    $("a[href='#next']").addClass("hidden").html("Siguiente").click(function () {
        if (step == 1) {
            ...
            $("a[href='#next']").addClass("hidden");
            $("a[href='#next']").html("Enviar");
            $("a[href='#next']").parent().removeClass("disabled");
        }
    });

    $("a[href='#finish']").off().html("Enviar").click(function () {
        if (validateCreateOrder()) {
            ...
            $.ajax({
                ...
                async: false,
                success: function (data) {
                    //window.location = "/Orders?msg=success&orderNumber=" + $("#no_orden").html();
                    window.location = "/Orders/Details/" + data.orderId + "?msg=success&orderNumber=" + $("#no_orden").html();
                }
            });
        }
    });
    ...
    var validateNuevoCliente = function () {
        ...
        if ($("#nuevoClientMovil").val() != "") {
            var regexPhone = /^(\+1 )?\(?\d{3}\)?[ -]\d{3}[ -]\d{4}$/;
            if (!regexPhone.test($("#nuevoClientMovil").val())) {
    ...
    var validateEditarCliente = function () {
        ...
        if ($("#inputClientMovil").val() != "") {
            var regexPhone = /^(\+1 )?\(?\d{3}\)?[ -]\d{3}[ -]\d{4}$/;
            if (!regexPhone.test($("#inputClientMovil").val())) {
                showWarningMessage("Atención", "El campo Teléfono no tiene el formato correcto. Formatos aceptados: +1 (123) 456-7899; (123) 456-7899; 123-456-7899; 123 456 7899.");
                return false;
            }
        }
    ...
    var validateEditarContacto = function () {
        ...
        if ($("#inputContactPhoneMovil").val() != "") {
            var regexPhoneMovil = /^(\+53 )?5\d{3}[ -]?\d{2}[ -]?\d{2}$/;
            if (!regexPhoneMovil.test($("#inputContactPhoneMovil").val())) {
                showWarningMessage("Atención", "El campo Teléfono Móvil no tiene el formato correcto. Formatos aceptados: +53 5xxxxxxx; 5xxx xx xx; 5xxx-xx-xx.");
                return false;
            }
        }

        if ($("#inputContactPhoneHome").val() != "") {
            var regexPhoneMovil = /^(\+53 )?((\d{1}[ ]?\d{3})|(\d{2}[ ]?\d{2}))[ -]?\d{2}[ -]?\d{2}$/;
            if (!regexPhoneMovil.test($("#inputContactPhoneHome").val())) {
                showWarningMessage("Atención", "El campo Teléfono Casa no tiene el formato correcto. Formatos aceptados: +53 7xxxxxxx; +53 xx xxxxxx; 7xxx-xx-xx; xx xxxxxx.");
                return false;
            }
        }
