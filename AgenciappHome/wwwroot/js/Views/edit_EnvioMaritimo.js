$(document).ready(function () {
    ...
    var addProductToTable = function (
        ...
        $(row).attr("data-id", productoBodegaId);

        if (productoBodegaId != "" && productoBodegaId != null) {
    ...
    var sendOrder = function () {
        if (validateCreateOrder()) {
            ...
            for (var i = 0; i < tBody.rows.length - 1; i++) {
                ...
                listProduct[i][2] = $(fila.children[4]).data("id");
                listProduct[i][3] = $(fila).attr("data-id");
                listProduct[i][4] = $(fila.children[2]).html();
            }
