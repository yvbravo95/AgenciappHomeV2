...
$(window).on("load", () => {
    ...
    tableIniciada = t.DataTable({
        ...
        ajax: {
            "url": `/empleadocubacargaam/GetData?status=TransitoCuba`,
            "type": 'POST',
            "dataType": "json",
        ...
        columnDefs: [
            ...
            },
            {
                ...
                name: "numero",
                render: function (data, type, item) {
                    return `<a href="/ordercubiq/detailbag?id=${item.paqueteId}">${data}</a>`;
                }
            },
            {
                targets: [3],
            ...
            },
            {
                ...
                render: function (data, type, item) {
                    ...
                    html.push(`<div class="dropdown-menu" aria-label="dropdown${data}">`);
...
function LoadEvents() {
    ...
    $(document).on("change", ".order-select", function () {
        ...
        if (cantSelect > 0) {
            $('#btn_recibir').removeClass('hidden');
        }
        else {
            $('#btn_recibir').addClass('hidden');
        }
    });

    $('#btn_recibir').on('click', function () {
        if (selectedIds.length == 0) {
            swal('Seleccione al menos un paquete', '', 'warning');
        }

        swal({
            title: "¿Está seguro?",
            text: "¿Desea marcar como recibidas las órdenes seleccionadas?",
            type: "warning",
            ...
            if (isConfirm) {
                $.ajax({
                    url: '/empleadocubacargaam/ChangeStatusBag',
                    type: 'POST',
                    data: {
                        status: "RecibidoAlmacen",
                        ids: selectedIds
                    },
                    success: function (data) {
                        if (data.success) {
                            swal("¡Correcto!", "Las órdenes seleccionadas se han recibido", "success");
                            tableIniciada.ajax.reload();
                            cantSelect = 0;
                            selectedIds = new Array;
                            $('#btn_distribuir').addClass('hidden');
                        }
                        else {
                            console.log(data);
                            swal("¡Error!", "Ha ocurrido un error al intentar recibir los paquetes", "error");
                        }
                    },
                    error: function (data) {
                        console.log(data);
        ...
        if (value && almacen) {
            $.ajax({
                ...
                data: {
                    status: "TransitoCuba",
                    ids: [value],
                    warehouseLocation: almacen
                },
                success: function (data) {
                    if (data.success) {
                        toastr.success("La orden se ha agregado");
                        tableIniciada.ajax.reload();
                    }
                    else {
                        console.log(data);
                ...
                error: function (data) {
                    console.log(data);
                    swal("¡Error!", "Ha ocurrido un error al intentar agregar el paquete", "error");
                    $('#inputBarCode').val('');
                }
            });
        }
        else {
            toastr.error("Verifique que se haya seleccionado el almacén");
            $('#inputBarCode').val('');
        }
    })
}
