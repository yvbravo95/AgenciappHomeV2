...
$(document).ready(function () {
    ...
    **********************************/
            ...
            success: function (data) {
                ...
                if (data.length == 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        $("#ContactId").append(new Option(contactData, data[i].contactId));
                    }
                    ...
                    if (selectedContact != null && selectedContact != "00000000-0000-0000-0000-000000000000") {
                        ...
                        $("#ContactId").val(last.contactId).trigger("change");
    ...
    function changeTarifa() {
        ...
        // Verifico que la agencia no sea Viaje Pronto
        if (agencyLegalName != "Viaje Pronto Inc") {
    ...
    var calc = function () {
        if (blockRight == "True") {
            ...
            var total = addPrecio;
            if (tarifa == "fijo") {
                total += monto + parseFloat(porciento);
            }
            else if (tarifa == "porciento") {
                total += monto + (monto * (parseFloat(porciento) / 100));
            }
    ...
    var calcBlockRight = function () {
        ...
        total += feeReal;
        var balanceValue = total - valorPagado;
    ...
    $('#btnAcceptCard').click(function () {
        if (!isCard) {
            ...
            // $('#isUsd').prop('checked', true);
            //Selecciono el myorista
    ...
    $('#btnCancelCard').click(function () {
        if (isCard == true) {
            swal({
                ...
                text: "Si cancela esta acción la tarjeta no se guardará",
                type: "warning",
                showCancelButton: true,
    ...
    $('#isRemittanceCard').trigger('change')
    ...
    if ($('#tipoPago').children("option:selected").html() == "Crédito o Débito") {
        $('#contfee').show();
        $('#AddAuthorization').show();
        calc();
    }
