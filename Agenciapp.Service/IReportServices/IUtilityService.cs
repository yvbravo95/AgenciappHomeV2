...
namespace Agenciapp.Service.IReportServices
{
    ...
    {
        Task<List<UtilityModel>> GetByService(Guid agencyId, STipo type, DateTime dateIni, DateTime dateEnd, bool onlyCanceled = false);
        Task<List<UtilityModel>> getAllUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled = false);
        Task<List<UtilityModel>> PassportUtilityByOrderNumber(Guid agencyId, string firstNumber, string secondNumber);
        Task<List<UtilityModel>> ServicioUtilityByOrderNumber(Guid agencyId, string firstNumber, string secondNumber);
    }
    public class UtilityService : IUtilityService
    {
        private readonly databaseContext _context;
        public UtilityService(databaseContext context)
        {
            _context = context;
        }
        ...
        public async Task<List<UtilityModel>> getAllUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled = false)
        {
            ...
            response.AddRange(await GetByService(agencyId, STipo.PTuristico, dateIni, dateEnd, onlyCanceled));
        ...
        private async Task<List<UtilityModel>> TicketUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var boletos = await _context.Ticket
                ...
                .Where(x => (onlyCanceled && x.State == Ticket.STATUS_CANCELADA) || (!onlyCanceled && x.State != Ticket.STATUS_CANCELADA))
                .Select(x => new
                {
                    x.TicketId,
                    x.Agency,
            ...
            foreach (var item in boletos)
            {
                ...
                if (AgencyName.IsDistrictCuba(agencyId) || AgencyName.MiamiPlusService == agencyId)
                {
        ...
        private async Task<List<UtilityModel>> MaritimeShippingUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var em = await _context.EnvioMaritimo
                        .AsNoTracking()
                        .Where(x => x.AgencyId == agencyId && x.Date.Date >= dateIni.Date && x.Date.Date <= dateEnd.Date)
                       .Where(x => (onlyCanceled && x.Status == EnvioMaritimo.STATUS_CANCELADA) || (!onlyCanceled && x.Status != EnvioMaritimo.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.Id,
                            x.Agency,
            ...
            {
                List<UtilityModel.Pay> newPays = new List<UtilityModel.Pay>();
                if (AgencyName.IsDistrictCuba(agencyId) || agencyId == AgencyName.MiamiPlusService)
                {
                    bool costIsLower = item.Pays.Any(x => x.PaidValue >= item.costoMayorista);
                    ...
                    foreach (var pay in item.Pays.OrderByDescending(x => x.PaidValue))
                    {
                        ...
                        if (costIsLower)
                        {
                            if (pay.PaidValue >= item.costoMayorista)
                            {
                                aux.Utility = pay.PaidValue - item.costoMayorista;
                                aux.Costo = item.costoMayorista;
                            }
                            else
                            {
                                aux.Utility = pay.PaidValue;
                        ...
                        {
                            decimal porcientoDelTotal = (pay.PaidValue * 100) / item.Amount;
                            decimal valorPorcientoEnCosto = (porcientoDelTotal * item.costoMayorista / 100);
                            aux.Utility = pay.PaidValue - valorPorcientoEnCosto;
                            aux.Costo = valorPorcientoEnCosto;
                        }
                        newPays.Add(aux);
        ...
        private async Task<List<UtilityModel>> AirShipmentUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var enviosaereos = await _context.Order
                        ...
                        .Where(x => (onlyCanceled && x.Status == Order.STATUS_CANCELADA) || (!onlyCanceled && x.Status != Order.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.Minorista,
                            x.OrderId,
                            ...
                            Client = new
                            {
                                Id = x.Client != null ? (Guid?)x.ClientId : null,
                                FullName = x.Client != null ? x.Client.FullData : "",
                                PhoneNumber = x.agencyTransferida == agency ? x.Agency.Phone.Number : x.Client.Phone.Number
                            },
                            Employee = new
                            {
                                Id = x.User != null ? (Guid?)x.User.UserId : null,
                        ...
                        .ToListAsync();
            List<UtilityModel> response = new List<UtilityModel>();
            var payTransferenciaBancaria = _context.TipoPago.FirstOrDefault(x => x.Type == "Transferencia Bancaria");
                    ...
                    else
                    {
                        ...
                        else
                        {
                            UtilityModel.Pay aux = new UtilityModel.Pay
                            {
                                ...
                                Id = Guid.Empty,
                                PaidValue = refprecio,
                                TipoPago = payTransferenciaBancaria.Type,
                                TipoPagoId = payTransferenciaBancaria.TipoPagoId,
                                Utility = refutilidad
                            };
                            newPays.Add(aux);
                        }
        ...
        private async Task<List<UtilityModel>> CaribbeanShippinUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var envioscaribe = await _context.EnvioCaribes
                        ...
                        .Where(x => (onlyCanceled && x.Status == EnvioCaribe.STATUS_CANCELADA) || (!onlyCanceled && x.Status != EnvioCaribe.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.EnvioCaribeId,
                            x.Agency,
        ...
        {
            var agency = await _context.Agency.FindAsync(agencyId);
            dateIni = dateIni.Date.ToUniversalTime();
            dateEnd = dateEnd.Date.AddDays(1).ToUniversalTime();
            var servicios = await _context.Servicios
                .AsNoTracking()
                .Include(x => x.Minorista)
                .Where(x => x.PaqueteTuristicoId == null && x.agency.AgencyId == agencyId && x.fecha >= dateIni && x.fecha <= dateEnd)
                .Where(x => (onlyCanceled && x.estado == Servicio.EstadoCancelado) || (!onlyCanceled && x.estado != Servicio.EstadoCancelado))
                .Select(x => new
                {
                    x.ServicioId,
                    x.agency,
                    ...
                    SubServicio = x.SubServicio.Nombre,
                    AgencyTransferida = new
                    {
                        AgencyId = (Guid?)null,
                        Name = ""
                    ...
                    Minorista = x.Minorista == null ? "" : x.Minorista.Name
                })
                .ToListAsync();
            List<UtilityModel> response = new List<UtilityModel>();
            foreach (var item in servicios)
            {
                decimal costo = item.costoMayorista + item.CostoXServicio;
                decimal utilidad = item.importeTotal - costo;
        ...
        private async Task<List<UtilityModel>> RechargueUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var recargas = await _context.Rechargue
                .AsNoTracking()
                .Where(x => x.AgencyId == agencyId && x.date.Date >= dateIni.Date && x.date.Date <= dateEnd.Date)
                .Where(x => (onlyCanceled && x.estado == Rechargue.STATUS_CANCELADA) || (!onlyCanceled && x.estado != Rechargue.STATUS_CANCELADA))
                .Select(x => new
                {
                    x.RechargueId,
                    x.Agency,
        ...
        private async Task<List<UtilityModel>> RemittanceUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var re = await _context.Remittance
                        ...
                        .Where(x => (onlyCanceled && x.Status == Remittance.STATUS_CANCELADA) || (!onlyCanceled && x.Status != Remittance.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.RemittanceId,
                            x.Agency,
            ...
            {
                decimal cost = item.costoMayorista + agency.remesa_entregaCuba + item.costoporDespacho;
                ...
                List<UtilityModel.Pay> newPays = new List<UtilityModel.Pay>();
                if (!isbytransferencia && (AgencyName.IsDistrictCuba(agencyId) || AgencyName.MiamiPlusService == agencyId))
                {
                    bool costIsLower = item.Pays.Any(x => x.PaidValue >= cost);
        ...
        private async Task<List<UtilityModel>> PTuristicoUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var re = await _context.PaquetesTuristicos
                        ...
                        .Where(x => (onlyCanceled && x.Status == PaqueteTuristico.STATUS_CANCELADA) || (!onlyCanceled && x.Status != PaqueteTuristico.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.PaqueteId,
                            x.Agency,
        ...
        private async Task<List<UtilityModel>> PassportUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            var pasaportes = await _context.Passport
                        ...
                        .Where(x => (onlyCanceled && x.Status == Passport.STATUS_CANCELADA) || (!onlyCanceled && x.Status != Passport.STATUS_CANCELADA))
                        .Select(x => new
                        {
                            x.PassportId,
                            x.Agency,
            ...
            if (AgencyName.IsDistrictCuba(agencyId))
            {
            ...
            foreach (var item in pasaportes)
            {
                ...
                if (item.AgencyTransferida.agencyId != null)
                {
                    ...
                    {   
                        ...
                        if(item.AgencyTransferida.agencyId == AgencyName.DCubaWashington)
                        {
                ...
                response.Add(new UtilityModel
                {
                    ...
                    Pays =  newPays.Any() ? newPays : item.Pays,
        ...
        private async Task<List<UtilityModel>> CubiqUtility(Guid agencyId, DateTime dateIni, DateTime dateEnd, bool onlyCanceled)
        {
            ...
            .Where(x => (onlyCanceled && x.Status == OrderCubiq.STATUS_CANCELADA) || (!onlyCanceled && x.Status != OrderCubiq.STATUS_CANCELADA))
            .Select(x => new
            {
                x.OrderCubiqId,
                x.Number,
            ...
            foreach (var item in envioscubiq)
            {
                ...
                if (item.AgencyTransferida.agencyId != null)
                {
                    if (item.AgencyTransferida.agencyId == agency.AgencyId)
                    {
                        ...
                        precio = item.costoMayorista + item.HandlingAndTransportation.Cost + item.OtrosCostos + item.InsuranceValue + item.ValorAduanal;
                        utilidad = precio - costo;
                    }
                    else
                    {
                        costo = item.costoMayorista + item.HandlingAndTransportation.Cost + item.OtrosCostos + item.InsuranceValue + item.ValorAduanal;
        ...
        public async Task<List<UtilityModel>> ServicioUtilityByOrderNumber(Guid agencyId, string firstNumber, string secondNumber)
        {
            ...
            var servicios = await _context.Servicios
                .AsNoTracking()
                .Where(x => x.estado != "Cancelado" && x.agency.AgencyId == agencyId)
                .Where(x => x.getNumberToValue() != -1 && x.getNumberToValue() >= number1 && x.getNumberToValue() <= number2)
                .Select(x => new
                {
                    x.ServicioId,
